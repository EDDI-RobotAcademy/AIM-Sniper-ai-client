...
class InterviewPreprocessingOpenAIRepositoryImpl(InterviewPreprocessingOpenAIRepository):
    ...
    def getTechKeyword(self, role):
        question = f"{role}가 알아야 할 핵심 기술 60가지를 알려줘."
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": f"너는 {role}야."},
                {"role": "user",
                 "content": f'{question} 출력 결과는 키워드<s>키워드<s>키워드<s>...키워드 형태로 해줘'
                 }
            ]
        )
        return response.choices[0].message.content.strip()
    ...
    def getTechAnswer(self, question, score):
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "너는 Backend 개발자야."},
                {"role": "user",
                 "content":
                     """[Question] """ + question +
                     f"""[Note]
                     ...
                     5. output은 다른 텍스트 없이 "answer:~<s>feedback:~<s>alternative_answer:~" 형식으로만 출력해줘.
                     """
